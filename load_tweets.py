...
def remove_nulls(s):
    ...
    Technically, this means the data in postgres won't be an exact match of
    the data in twitter, because we are not also escaping the escaped strings.
    The added complexity of a fully working escaping system doesn't seem worth
    the benefits since null characters appear so rarely in twitter text.
    '''
    if s is None:
        return None
    else:
...
def insert_tweet(connection, tweet):
    ...
    with connection.begin() as trans:
        ...
        for mention in mentions:
            ...
            # note that we already have done an insert into the users table
            # above for the user who sent a tweet;
            # that insert had lots of information inside of it (i.e. the user
            # row was "hydrated");
            # when we only have a mention of a user, however, we do not have
            # all the information to store in the row;
...
# NOTE:
# we reverse sort the filenames because this results in fewer updates
# to the users table,
